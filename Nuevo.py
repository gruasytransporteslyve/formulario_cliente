...
def app():
    ...
    if transformed_name in names_list and transformed_name:
        ...
        st.text_input("Ruta potencial para este cliente",disabled=True)
        st.number_input("Precio unitario de salida", disabled=True, min_value=0)
        st.number_input("Precio por kilómetro", disabled=True, min_value=0)
        st.number_input("Precio unitario de trabajo de grúa", disabled=True, min_value=0)
        st.number_input("Precio unitario por descarga", disabled=True, min_value=0)
        st.number_input("Precio de servicio mínimo", disabled=True, min_value=0)
        ...
        if not email_warning:
            ...
            n_employees = st.number_input("Número de empleados:", step=1, min_value=0,value=None)
