...
class Load:
    ...
    def upload_to_drive(file_name, mime_type, folder_id, company_name) -> str:
        """
        ...
        # Check if the subfolder (with company_name) exists
        query = f"'{folder_id}' in parents and mimeType = 'application/vnd.google-apps.folder' and name = '{company_name}' and trashed = false"
    ...
    def generate_albaran_id(albaran_df):
        # Get the current year as the last two digits
        current_year_suffix = str(datetime.datetime.now().year)[-2:]
        try:

            # Filter the albaran_id column for the current year's suffix
            matching_ids = albaran_df['albarán_id'].dropna().astype(str).str.endswith(f"/{current_year_suffix}")

            # Count how many IDs match the current year's suffix
            count = matching_ids.sum()

        except:
            count=0
        # Generate the new albaran_id
        new_id = f"{count + 1}/{current_year_suffix}"
        return new_id
